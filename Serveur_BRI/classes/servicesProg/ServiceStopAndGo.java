...
public class ServiceStopAndGo implements Runnable {
    private Socket client;
    private String login;

    public ServiceStopAndGo(Socket socket, String login) {
        client = socket;
        this.login = login;
    }
    ...
    public void run() {
        try {
            ...
            out.println(ServiceRegistry.toStringue(login) + "#nPlease choose the one to Stop or Start");
            ...
            if (ServiceRegistry.getNumberOfService(login) != 0) {
                while (badFormat) {
                    try {
                        ...
                        badFormat = false;
                        ServiceRegistry.setClassState(numService);
                    } catch (Exception e) {
                        badFormat = true;
                    }
                }
